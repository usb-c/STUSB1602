...
USBPD_StatusTypeDef USBPD_PWR_IF_SetProfile(uint8_t PortNum, uint8_t Profile, uint8_t PreviousPowerProfile)
{
  ...
  if (USBPD_PORTPOWERROLE_SRC == DPM_Params[PortNum].PE_PowerRole)
  {
    ...
    if(pdo.GenericPDO.PowerObject == USBPD_CORE_PDO_TYPE_APDO)
    {
      ...
      current_measured = APPLI_GetIvbus(PortNum);                 /* Get current drawn by sink */
...
/**end of adjust target value vs current drawn from sink */
          ...
          if ( (delta_val > step) && (pos == 0))
          {
            do
            {
             ...
             APPLI_SetVoltage(PortNum, new_val);
              ...
              osDelay(transition);
              while  (((  APPLI_GetVBUS(PortNum)  ) > max ) || ((  APPLI_GetVBUS(PortNum)) < min )) ;//&& (Ivbus > DPM_Ports[PortNum].DPM_RequestedCurrent + 150))
              {
                {
#ifdef _TRACE
                  uint8_t tab[32];
                ...
                {
#ifdef _TRACE
                  USBPD_TRACE_Add(USBPD_TRACE_DEBUG, PortNum, 0, "Timeout", sizeof("Timeout"));
#endif
                  return USBPD_ERROR;
                }
              }
  ...
  USBPD_TRACE_Add(USBPD_TRACE_DEBUG, PortNum, 0, tab, size); 
#endif   
  //       if (1 == DPM_Ports[PortNum].DPM_flag_exitCC)
...
USBPD_StatusTypeDef USBPD_PWR_IF_SupplyReady(uint8_t PortNum, USBPD_VSAFE_StatusTypeDef Vsafe)
{
  ...
  {
    /* Vsafe0V */
//          status = (APPLI_GetVBUS(PortNum) < CAD_threshold_VBus ? USBPD_OK: USBPD_ERROR);
          Vbus_board = APPLI_GetVBUS(PortNum);
          status = (Vbus_board < 800 ? USBPD_OK: USBPD_ERROR);
// #ifdef _TRACE
//        uint8_t tab[32];
//        uint8_t size;
//        size = sprintf((char*)tab, "VBusMeas: %d", Vbus_board);
//        USBPD_TRACE_Add(USBPD_TRACE_DEBUG, PortNum, 0, tab, size); 
// #endif /* _TRACE   */
  }
  else
...
USBPD_StatusTypeDef USBPD_PWR_IF_VBUSEnable(uint8_t PortNum)
{
  ...
  {
#if defined(_TRACE)
    USBPD_TRACE_Add(USBPD_TRACE_DEBUG, PortNum, 0, "EN_VBUS", 7);
    char str[20];
    ...
    /* Set the new state */
          ...
          /* Power Enable is handle featue directly by the device */
...
USBPD_StatusTypeDef USBPD_PWR_IF_VBUSDisable(uint8_t PortNum)
{
  ...
  {
#if defined(_TRACE)
    USBPD_TRACE_Add(USBPD_TRACE_DEBUG, PortNum, 0, "DIS VBUS", 8);
#endif /* _TRACE */
    /* Set the new state */
          /* Power Enable is handle featue directly by the device */
...
USBPD_StatusTypeDef USBPD_PWR_IF_SNKDisable(uint8_t PortNum)
{
  ...
  if (USBPD_PORT_IsValid(PortNum))
  {
          /* Set the new state */ /* Power Enable is handle featue directly by the device */
...
USBPD_StatusTypeDef USBPD_PWR_IF_Enable_VConn(uint8_t PortNum, CCxPin_TypeDef CC)
{
  USBPD_StatusTypeDef _status = USBPD_OK;
#if defined(_TRACE)
  USBPD_TRACE_Add(USBPD_TRACE_DEBUG, PortNum, 1,"VCONN ON", 8);
#endif /* _TRACE */
...
USBPD_StatusTypeDef USBPD_PWR_IF_Disable_VConn(uint8_t PortNum, CCxPin_TypeDef CC)
{
  USBPD_StatusTypeDef _status = USBPD_OK;
#if defined(_TRACE)
  USBPD_TRACE_Add(USBPD_TRACE_DEBUG, PortNum, 1,"VCONN OFF", 9);
#endif /* _TRACE */
...
void USBPD_PWR_IF_GetPortPDOs(uint8_t PortNum, USBPD_CORE_DataInfoType_TypeDef DataId, uint8_t *Ptr, uint32_t *Size)
{
  ...
  if (USBPD_PORT_IsValid(PortNum))
  {
    ...
    case USBPD_CORE_DATATYPE_SNK_PDO:
      if (USBPD_SPECIFICATION_REV2 == DPM_Params[USBPD_PORT_0].PE_SpecRevision)
      {
        PWR_Port_PDO_Storage[PortNum].SinkPDO.ListOfPDO = (uint32_t *)PORT0_PDO_ListSNK;
        PWR_Port_PDO_Storage[PortNum].SinkPDO.NumberOfPDO = PORT0_NB_SINKPDO;
      }
      else
        if (USBPD_SPECIFICATION_REV3 == DPM_Params[USBPD_PORT_0].PE_SpecRevision)
        {
          PWR_Port_PDO_Storage[PortNum].SinkPDO.ListOfPDO = (uint32_t *)PORT0_PDO_ListSNK;
          PWR_Port_PDO_Storage[PortNum].SinkPDO.NumberOfPDO = PORT0_NB_SINKPDO + PORT0_NB_SINKAPDO;
        }
    ...
    {
      {
            ...
            || (VBUS_3A == DPM_Ports[PortNum].VDM_DiscoCableIdentify.CableVDO.b.VBUS_CurrentHandCap))
        {
          ...
          if (localpdo.GenericPDO.PowerObject == USBPD_CORE_PDO_TYPE_APDO)
          {
            localpdo.SRCSNKAPDO.MaxCurrentIn50mAunits = USBPD_MIN(localpdo.SRCSNKAPDO.MaxCurrentIn50mAunits , USBPD_CORE_CABLE_3A / 50);
          }
          else
          {
            localpdo.SRCFixedPDO.MaxCurrentIn10mAunits = USBPD_MIN(localpdo.SRCFixedPDO.MaxCurrentIn10mAunits , USBPD_CORE_CABLE_3A / 10);
          }
          if (0 == index)
          {
            (void)memcpy(Ptr, (uint8_t*)&localpdo.d32, 4u);
          }
          else
          ...
          /* set OCP current limitation according to PDO and Cable */    
          //        APPLI_Set_Current_Limit(PortNum , localpdo.SRCFixedPDO.MaxCurrentIn10mAunits) ;
        ...
        else
        {
          if (USBPD_PORTPOWERROLE_SRC ==DPM_Params[PortNum].PE_PowerRole)
          {
            if (VBUS_5A == DPM_Ports[PortNum].VDM_DiscoCableIdentify.CableVDO.b.VBUS_CurrentHandCap)
            {
              ...
              if (localpdo.GenericPDO.PowerObject == USBPD_CORE_PDO_TYPE_APDO)
              {
                localpdo.SRCSNKAPDO.MaxCurrentIn50mAunits = USBPD_MIN(localpdo.SRCSNKAPDO.MaxCurrentIn50mAunits , USBPD_CORE_CABLE_5A / 50);
              }
              else
              {
                localpdo.SRCFixedPDO.MaxCurrentIn10mAunits = USBPD_MIN(localpdo.SRCFixedPDO.MaxCurrentIn10mAunits , USBPD_CORE_CABLE_5A / 10);
              }
//                        USPBPD_WRITE32((uint32_t *)(Ptr + nb_valid_pdo), localpdo.d32);    
              if (0 == index)
              {
                (void)memcpy(Ptr, (uint8_t*)&localpdo.d32, 4u);
              }
...
//                USPBPD_WRITE32((uint32_t *)(Ptr + nb_valid_pdo), *ptpdoarray);
            ...
            else
            {
              ...
              /* set OCP current limitation according to PDO and Cable */    
              //     APPLI_Set_Current_Limit(PortNum , DPM_Ports.SRCFixedPDO.MaxCurrentIn10mAunits); 
            }
          }
          else /*PE_PowerRole = Sink */
          {
