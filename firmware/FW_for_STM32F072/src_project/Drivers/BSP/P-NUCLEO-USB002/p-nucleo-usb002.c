...
uint16_t APPLI_GetVBUS(uint8_t PortNum)
{
  ...
  vbus_board = (uint16_t)MVOLTAGE( ADCxConvertedValues[VBUS_INDEX(PortNum)] );
...
//  val1 = (uint16_t) (4096*225/330);
//  val3 = ADCxConvertedValues[IBUS_INDEX(PortNum)];
//  val2 = (uint32_t) (val3 * 3000  / val1);
