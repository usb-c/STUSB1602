...
******************************************************************************
*/
#ifndef P_NUCLEO_L4R5_C
#define P_NUCLEO_L4R5_C 
...
const uint8_t HWBoardVersionName[] = "P-Nucleo-L4R5";
...
void HW_IF_ADC_Init(void)
{
  ...
  usbpdm1_hadc.Init.ScanConvMode = ENABLE;  
  usbpdm1_hadc.Init.EOCSelection = ADC_EOC_SINGLE_CONV;
  usbpdm1_hadc.Init.LowPowerAutoWait = DISABLE;
//  usbpdm1_hadc.Init.NbrOfConversion = 1; /* Parameter discarded because sequencer is disabled */
...
//#endif   
  usbpdm1_hadc.Init.DiscontinuousConvMode = DISABLE; /* Parameter discarded because sequencer is disabled */
  usbpdm1_hadc.Init.NbrOfDiscConversion = 0; /* Parameter discarded because sequencer is disabled */
  usbpdm1_hadc.Init.NbrOfConversion = ADCCONVERTEDVALUES_BUFFER_SIZE; /* Parameter discarded because sequencer is disabled */
  usbpdm1_hadc.Init.DMAContinuousRequests = ENABLE; /* DMA circular mode selected */
  usbpdm1_hadc.Init.Overrun = ADC_OVR_DATA_PRESERVED; /* DR register is overwritten with the last conversion result in case of overrun */
  usbpdm1_hadc.Init.OversamplingMode = DISABLE; /* Oversampling enabled */
  usbpdm1_hadc.Init.ContinuousConvMode = ENABLE; /* Continuous mode enabled (automatic conversion restart after each conversion) */
  usbpdm1_hadc.Init.ExternalTrigConv = ADC_SOFTWARE_START; /* Software start to trig the 1st conversion manually, without external event */
  usbpdm1_hadc.Init.ExternalTrigConvEdge = ADC_EXTERNALTRIGCONVEDGE_NONE; /*Parameter discarded because software trigger chosen */
  usbpdm1_hadc.Init.Oversampling.Ratio = ADC_OVERSAMPLING_RATIO_64; /* Oversampling ratio */
  usbpdm1_hadc.Init.Oversampling.RightBitShift = ADC_RIGHTBITSHIFT_2; /* Right shift of the oversampled summation */
  usbpdm1_hadc.Init.Oversampling.TriggeredMode = ADC_TRIGGEREDMODE_SINGLE_TRIGGER; /* Specifies whether or not a trigger is needed for each sample */
  usbpdm1_hadc.Init.Oversampling.OversamplingStopReset = ADC_REGOVERSAMPLING_CONTINUED_MODE;
  /* Initialize ADC peripheral according to the passed parameters --------*/
  if (HAL_ADC_Init(&usbpdm1_hadc) != HAL_OK) 
  {
//    Error_Handler();
  }
  /* Start calibration ----------------------------------------------------*/
//  if (HAL_ADCEx_Calibration_Start(&usbpdm1_hadc, ADC_SINGLE_ENDED) != HAL_OK)
  {
//    Error_Handler();
  }
...
#if _ADC_MONITORING
#warning "if I Vus is use please Verify ADC accuracy "
#endif   
#if _ADC_MONITORING_na  
